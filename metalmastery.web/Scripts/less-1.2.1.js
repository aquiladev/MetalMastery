    ...
    (function (tree) {
        ...
        tree.functions = {
            ...
            xxx: function () {
                ...
                var li = 0.4;
                var b = 1
                var s = (Math.random() * li + dev) * 0.3;
                var a = Math.random() * (b * s * 255);

                var rgb = [];
                rgb[0] = Math.round(b * 255);
                rgb[1] = Math.round((b * 255) - (b * s * 255));
                rgb[2] = Math.round(b * 255 - a);

                for (var j, x, i = rgb.length; i; j = parseInt(Math.random() * i), x = rgb[--i], rgb[i] = rgb[j], rgb[j] = x);

                var color = new tree.Color(rgb);
                var hsl = color.toHSL();
                return hsla(hsl);
            },
